...
const Doctor_Register = () => {
    ...
    const HandleRegister = async (e)=>{
        ...
        setLoading(true);
        try {
            const response = await fetch(`${baseUrl}/doctor/createDoctor`,{
            method:"POST",
            headers:{
  ...
  return (
    <div className="flex justify-center items-center w-full my-auto mt-12">
        <form onSubmit={HandleRegister} className="shadow-xl flex justify-center flex-col border-[.5px] rounded-lg gap-y-5 items-center p-4">
             <h1 className="uppercase text-2xl font-bold tracking-widest break-words text-black">Register as <span className="text-hover">DOCTOR</span></h1>
            ...
            <Link to={"/login/doctor"} className="text-card"><span className="mx-2 text-black">Already have an account</span>Login Here</Link>
